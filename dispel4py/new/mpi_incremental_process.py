...
def coordinator(workflow: WorkflowGraph, inputs, args):
    ...
    def onRequire(target: str, source_rank: int, workflow: WorkflowGraph, task_list: TaskList, outputmappings: IOMapping):
        ...
        for (linked_wfNode, attributes) in workflow.graph[source_wfNode].items():
            if attributes['DIRECTION'][0] == source_pe:
                for fromConnection, _ in attributes['ALL_CONNECTIONS']:
                    if fromConnection == target:
                        target_wfNode = linked_wfNode
                        has_next = True
                        dbg3("[coordinator] target_wfNode found: {}".format(target_wfNode))
                        break
            if has_next:
                break
        if not has_next:
            dbg1("[coordinator] encountered finial node of stream: {} [{}]".format(target, source_rank))
            comm.send([], source_rank, tag=TAG_TARGET)
            dbg1("[coordinator] targets:{} sent to {}".format([], source_rank))
            return
        dbg4("[coordinator] getting pe from wfNode")
        required_pe = target_wfNode.getContainedObject()
        dbg2("[coordinator] required_pe: {}".format(required_pe))
        dbg3("[coordinator] looking up if exists")
        ...
        dbg1("[coordinator] sending brothers to {}".format(source_rank))
        comm.send(indices, source_rank, tag=TAG_TARGET)
        dbg1("[coordinator] brothers sent: {}".format(indices))
        if source_rank not in outputmappings or target not in outputmappings[source_rank]:
            dbg2("[coordinator] source ({}) or target ({}) is not in outputmappings, needs update".format(source_rank, target))
            for node_rank, mappings in setup_outputmappings(source_pe, required_pe, indices).items():
                if node_rank not in outputmappings:
                    outputmappings[node_rank] = mappings
                else:
                    outputmappings[node_rank].update(mappings)
        ...
        comm.send(outputmappings[source_rank], source_rank, tag=TAG_OUTPUT_MAPPING)
...
def executor(workflow, inputs, args):
    ...
    while True:
        ...
        if tag == TAG_DEPLOY:
            ...
            wrapper = MPIIncWrapper(pe, provided_inputs)
...
class MPIIncWrapper(GenericWrapper):
    ...
    def request_target(self, target: str):
        ...
        self.inform_output(target, output_mapping[target]) #FIXME: remove either the index or parameter (target)
    ...
    def _read(self):
        ...
        if result is not None:
            dbg1("[{}] _read returning".format(rank))
            return result
    ...
    def _write(self, name, data):
        ...
        for (inputName, communication) in targets:
            dbg3("[{}] inputName:{} communication:{}".format(rank, inputName, communication))
