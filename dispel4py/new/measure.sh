...
function step {
	...
	/usr/bin/time -f %e mpiexec -np $np dispel4py mpi_inc dispel4py.examples.graph_testing.measure.repeatable_prime_sieve -i $number_of_iteration > /dev/null 2> time_mpi_inc &&
	echo "=================" &&
	/usr/bin/time -f %e mpiexec -np $(($max_number_of_sieves+1)) dispel4py mpi dispel4py.examples.graph_testing.measure.repeatable_prime_sieve__static_$max_number_of_sieves -i $number_of_iteration > /dev/null 2> time_mpi &&
...
for ((i=0;i<$length;i++)) do
	...
	for ((number_of_iteration=1;number_of_iteration<100;number_of_iteration+=10)); do
		for ((i=0;i<8;i+=1)); do step $np $number_of_iteration $max_sieve $max_prime; done
	done
done
